...
public interface ChildVaccineRepository extends JpaRepository<ChildVaccine, Long> {
    ...
    @Query("SELECT new hackathon.medhack.backend.model.dto.ChildVaccineDto(va.id, cv.id, va.name, va.abbreviation, va.description, cv.childVaccineDate, cv.dateWhenDone, cv.isDone) " +
