...
const TabView = ({children, activeTab}) => {
    const tabs = Array.isArray(children)
        ...
        : children?.type?.name === 'TabPanel' ? [children] : [];

    const headerButtons = tabs.map((tab) => tab.props?.title || '_');
    const activeTabComputed = headerButtons.indexOf(activeTab) > -1 ? headerButtons.indexOf(activeTab) : 0;
            ...
            <ul className="med-tabs__header">
                {
                    headerButtons.map((title, index) => {
                        return (
                            <MedButton onClick={() => setSelectedTab(index)}
                                       label={title}
                                       variant='plain'
